...
if __name__ == '__main__':
    try:
        ...
        for k in range(num_points):
            ...
            v = np.sqrt(xd[3,k]**2 + xd[4,k]**2)*np.cos(e[2,k+1]) + kp_v*e[0,k+1] 
            v = sat(max_turn, v)
            omega = xd[5,k] + kp_y*np.sqrt(xd[3,k]**2 + xd[4,k]**2)*e[1,k+1] + kp_omega*np.sin(e[2,k+1])
...
df1.to_csv('CLF_field'+ str(trial_number) + '.csv', index=False)
